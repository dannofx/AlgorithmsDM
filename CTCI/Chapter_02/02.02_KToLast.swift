...
class Node<Element> where Element: Comparable {
    ...
    func getLastElement(offset: UInt) -> Node? {
        var index = 0
        var aheadPointer: Node? = self
        var delayedPointer: Node?
        while aheadPointer != nil {
            if index >= offset {
                if delayedPointer != nil {
...
head.printList()
let k: UInt = 9
if let foundNode = head.getLastElement(offset: k) {
    print("The element that is \(k) positions away from the end is: \(foundNode.value)")
} else {
    print("Error: There is not node \(k) positions away from the end.")
}
